...
prezto_apply_patches() (
  if [ "$(whoami)" != "root" ]; then
    ...
    patch -p1 -f -r - --no-backup-if-mismatch < "$HOME/dotfiles/zsh/zprezto-patches/prompt.patch" >/dev/null 2>&1
    patch -p1 -f -r - --no-backup-if-mismatch < "$HOME/dotfiles/zsh/zprezto-patches/wordchars.patch" >/dev/null 2>&1
  fi
)
